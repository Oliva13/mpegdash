...
static size_t fmp4_write_sidx(struct mov_t* mov)
{
	...
	for (i = 0; i < mov->track_count; i++)
	{
		...
		if (track->sample_count > 0)
		{
			...
			printf("fmp4_write_sidx : track->samples[%d].dts - %lld\n", track->sample_count-1, track->samples[track->sample_count-1].dts);
			printf("fmp4_write_sidx : track->samples[0].dts - %lld\n", track->samples[0].dts);
#endif			
		}
		else
		{
		...
		mov_buffer_w32(&mov->io, 52); /* size */ //my 52 -> 44
		mov_buffer_write(&mov->io, "sidx", 4);
		mov_buffer_w8(&mov->io,  1); /* version */  // my 1->0
		mov_buffer_w24(&mov->io, 0); /* flags */
		...
		//mov_buffer_w64(&mov->io, 44 * (mov->track_count - i - 1)); /* first_offset */ // my 52->44
		
		mov_buffer_w64(&mov->io, earliest_presentation_time); /* earliest_presentation_time */
		mov_buffer_w64(&mov->io, 52 * (mov->track_count - i - 1)); /* first_offset */ // my 52->44
		...
		mov_buffer_w32(&mov->io, (1U/*starts_with_SAP*/ << 31) | (0 /*SAP_type*/ << 24) | 0 /*SAP_delta_time*/);  // my //0-1 (0 /*SAP_type*/ << 24) 
#if defined(DEBUG_INFO)			
		printf("fmp4_write_sidx : track->mdhd.timescale - %d\n",track->mdhd.timescale);
		printf("fmp4_write_sidx : track->tkhd.track_ID - %d\n", track->tkhd.track_ID);
		printf("fmp4_write_sidx : earliest_presentation_time - %d\n", earliest_presentation_time);
		printf("fmp4_write_sidx : duration - %d\n", duration);
#endif		
	}
	return 52 * mov->track_count; // my 52->44
}
...
static int fmp4_write_fragment(struct fmp4_writer_t* writer)
{
	...
	//mov_write_hvcc_seg(mov); // if h265
...
static int fmp4_writer_init(struct mov_t* mov)
{
	...
	//	//mov->ftyp.major_brand = MOV_BRAND_AVC1;
	...
	//{
		...
		mov->ftyp.minor_version = 0;
		mov->ftyp.brands_count = 3;
		mov->ftyp.compatible_brands[0] = MOV_BRAND_ISOM;
		//mov->ftyp.compatible_brands[1] = MOV_BRAND_MP42;
		mov->ftyp.compatible_brands[1] = MOV_BRAND_AVC1;
		mov->ftyp.compatible_brands[2] = MOV_BRAND_DASH;
		mov->header = 0;
	//}
	return 0;
}
...
int fmp4_writer_add_video(struct fmp4_writer_t* writer, uint8_t object, int width, int height, const void* extra_data, size_t extra_data_size)
{
	...
	//track->mdhd.timescale = 90000; //mov->mvhd.timescale
	track->mdhd.language = 0x55c4;
	track->mdhd.duration = 0; // placeholder
