...
size_t mov_write_trun(const struct mov_t* mov, uint32_t flags, uint32_t flags0, size_t from, size_t count)
{
	...
	//printf("flags - 0x%x\n", flags);
	//printf("First sample flags - 0x%x\n", flags0);
	...
	for (i = from; i < from + count; i++)
	{
		...
		{
			//printf("compositional time offset - %d\n", (int32_t)(sample->pts - sample->dts));				
			mov_buffer_w32(&mov->io, (int32_t)(sample->pts - sample->dts)); /* sample_composition_time_offset */
			size += 4;
		}
	}
#if defined(DEBUG_INFO)
	printf("mov_write_trun : size - %d\n", size);
	printf("mov_write_trun : offset - %d\n", offset);
#endif	
